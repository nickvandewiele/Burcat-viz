function createMWPlot(dataset) {
	...
	var x = d3.scale.linear().domain([d3.min(values), d3.max(values)]).range([0, width]);
	...
	svg.append("text").attr("class", "x label").attr("text-anchor", "end").attr("x", width).attr("y", height - margin.bottom ).text("MW / g mol-1");
